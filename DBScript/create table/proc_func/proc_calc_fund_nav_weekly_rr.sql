CREATE PROCEDURE `proc_calc_fund_nav_weekly_rr`(
i_wind_code varchar(20), 
i_date_from date
)
BEGIN
	declare kdr_type int default 2; -- name_date_rr type = 2
    ...
    DECLARE cur CURSOR FOR 
		select nav_date_week, fv.nav_acc
		from (
			...
			having nav_date_week >= i_date_from
			) as ffv,
			fund_nav fv
		where fv.wind_code = ffv.wind_code
